...
class MutableNullableContextParamSerializationTest : FunSpec({
    val dataRight = MutableNullableContextParam<DealInfo, SomeError>(
        Arranger.some(DealInfo::class.java).right()
    )

    val dataLeft = MutableNullableContextParam<DealInfo, SomeError>(
        Arranger.some(SomeError::class.java).left()
    )


    val serializer = MutableNullableContextParam.serializer<DealInfo, SomeError>(
        DealInfo.serializer(),
